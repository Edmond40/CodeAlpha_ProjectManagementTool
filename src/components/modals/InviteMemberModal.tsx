...
const roleOptions = [
  ...
  { value: 'Member', label: 'Member', description: 'View and contribute' },
];
...
export function InviteMemberModal() {
  ...
  const canEditRole = editingMember?.role !== 'Application';
  ...
  useEffect(() => {
    if (editingMember && editingMember.role !== 'Application') {
      reset({
        name: editingMember.name,
        email: editingMember.email,
        role: editingMember.role as FormValues['role'],
      });
  ...
  const onSubmit = (data: FormValues) => {
    if (editingMember && canEditRole) {
      updateMember(editingMember.id, { name: data.name, email: data.email, role: data.role });
      addToast({ title: 'Member updated', type: 'success' });
    } else if (!editingMember) {
      addMember({ name: data.name, email: data.email, role: data.role });
      addToast({ title: 'Invitation sent', description: `Invite sent to ${data.email}.`, type: 'success' });
    }
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              <h2 className="text-xl font-bold text-foreground">
                {editingMember ? 'Edit member' : 'Invite people'}
              </h2>
              <p className="text-sm text-muted-foreground mt-0.5">
                {editingMember ? 'Update role and details.' : 'They will receive an invitation email.'}
              </p>
            </div>
            <button onClick={closeInviteMemberModal} className="p-1 rounded-lg hover:bg-muted text-muted-foreground">
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {canEditRole && (
              <Controller
                ...
                control={control}
                render={({ field }) => (
                  <Select label="Role" value={field.value} onChange={field.onChange} options={roleOptions} size="sm" />
                )}
              />
            )}

            <div className="flex justify-end gap-2 pt-4 border-t border-border">
              <Button type="button" variant="outline" onClick={closeInviteMemberModal}>
                Cancel
              </Button>
              <Button type="submit">{editingMember ? 'Save' : 'Send invite'}</Button>
            </div>
          </form>
        </motion.div>
      </div>
